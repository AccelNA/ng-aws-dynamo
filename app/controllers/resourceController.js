...
bookmarkApp.controller('resourceController',['$scope','$routeParams','resourceService', '$cookieStore','$location','$http','$resource','ngTableParams','$filter', function ($scope,$routeParams,resourceService, $cookieStore,$location, $http, $resource,ngTableParams,$filter) {
    ...
    $scope.fnAddresource = function (isValid) {
        if (isValid) {
        	...
        	result= resourceService.addresource($scope.userData);    
...
//edit resource
        $scope.fnEditresource = function (resource) {
	    	...
	    	$scope.Resource_id = resource.resources_id.S;
	    	$scope.Resource_name = resource.resources_name.S;
	    	discription = resource.resources_description.S;
	    	$scope.Resource_decription = discription ? discription: null;
	    	$scope.Resource_path = resource.resources_path.S;
	    	$scope.Resource_priority = resource.resources_priority.S;
	    	$scope.Resource_resourcegroup_name = resource.resources_group_name.S;
	    	$cookieStore.put('path', resource.resources_path.S);
